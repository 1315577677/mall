...
<div class="col-sm-9 col-sm-offset-3 col-md-10 col-md-offset-2 main">
    <div class=" panel panel-default">
        <form id="update" class="form-inline"action="<%=basePath%>getOrderInfo.action">
            <input hidden name="stutas" value="1">
            ...
            <table class="table text-detail">
                ...
                <c:forEach items="${page.rows}" var="row">
                    <tr >
                        <td><input type="checkbox" name="sendProduct" value="${row.ordernum}"></td>
                        <td>${row.ordernum}</td>
                        <td>${row.username}</td>
                        <td>${row.name}</td>
                        <td>${row.price}</td>
            ...
            <div class="text-right">
                <a onclick="backtodawn()" class="pull-left pagination btn btn-primary">发货</a>
    ...
    <script>
        function backtodawn() {
            var str = "";
            $("[name='sendProduct']:checked").each(function () {
                str += $(this).val() + ",";
            })

            if(confirm('确实要发送这些定单吗?')) {
                $.ajax({
                    type: 'get',
                    url: "<%=basePath%>backtodown.action",
                    data: {"str":str},
